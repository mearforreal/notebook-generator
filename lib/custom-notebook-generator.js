/**
 * Custom notebook generator with selectable cleaning steps
 * Supports dataset-specific operations like location splitting and salary parsing
 */
...
const stepGenerators = {
  ...
  load_csv: (csvPath) => createCodeCell(
    `# Load the CSV file\n` +
    `csv_path = r"${csvPath}"\n` +
  ...
  normalize_binary: () => createCodeCell(
  ...
  drop_duplicates: () => createCodeCell(
...
export function generateCustomNotebook(csvFilename, csvPath, selectedSteps) {
  const metadata = {
    kernelspec: {
  ...
  cells.push(createMarkdownCell(
    ...
    `**Selected cleaning steps:** ${Object.keys(selectedSteps).filter(k => selectedSteps[k]).join(', ')}`
  ));
  ...
  for (const step of stepOrder) {
    if (selectedSteps[step] && stepGenerators[step]) {
      cells.push(stepGenerators[step](csvPath));
    }
  }
